...
2. Baseline graph-health checks (total nodes, total relationships, labels/types, predicates, and property coverage)
3. Relationship metadata diagnostics
4. Database-level metadata/schema inspection (database context, labels, property keys, node/relationship schema and coverage)
5. Final ingestion validation across the three sources (Cultura y censura, fosas comunes, SIDBRINT) with: 
  6. Mapping integrity checks (A1–A3, B1, C1, X1, X2)
...
curl --fail-with-body --silent --show-error \
  ...
  -H 'Accept: application/json' \
  -d '{
    "query": "SELECT (COUNT(DISTINCT ?s) AS ?distinctSubjects) (COUNT(DISTINCT ?p) AS ?distinctPredicates) (COUNT(*) AS ?totalTriples) WHERE { ?s ?p ?o . }",
    "format": "json"
  }'
```
...
RETURN "label" AS category, label AS value
UNION ALL
SHOW RELATIONSHIP TYPES YIELD relationshipType
RETURN "relationshipType" AS category, relationshipType AS value
UNION ALL
...
```cypher
MATCH ()-[r]->()
WITH type(r) AS relType, count(*) AS totalRelationships
MATCH ()-[r]->()
WHERE type(r) = relType
UNWIND keys(r) AS propertyName
WITH relType, propertyName, totalRelationships, r[propertyName] AS v
WITH relType,
     propertyName,
     totalRelationships,
